...
Candle::Candle(CANdleBaudrate_E canBaudrate, bool _printVerbose, bool useLogs, mab::CANdleFastMode_E _fastMode, bool printFailure, mab::BusType_E busType)
{
	bus = new Bus(busType);
	...
	else if (bus->getType() == mab::BusType_E::SPI)  //additions for bus
	{
		...
		if (bus->transfer(tx, len, true, 50, 66))
			if (*bus->getRxBuffer(1) == true)
			{
				...
				else
				{
					...
					this->md80s.at(canId).__updateRegulatorsAdjusted(false);
				}
				return true;
			}
